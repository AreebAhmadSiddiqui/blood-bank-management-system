...
function RecipientsPage() {
    ...
    let totalBloodrequestd=0
    let requestsPending=0
    let totalRequest=0
    let requestApproved=0
    if(targetObj){
        // console.log(targetObj);
        totalBloodrequestd=targetObj.request.reduce((counter, obj) => counter += Number(obj.amount) , 0);
        // console.log(totalBloodrequestd);
        requestsPending=targetObj.request.reduce((counter, obj) => counter += obj.status.toLowerCase()==='pending' , 0);
        requestApproved=targetObj.request.reduce((counter, obj) => counter += obj.status.toLowerCase()==='approved' , 0);
        totalRequest=targetObj.request.reduce((counter, obj) => counter += 1 , 0);
    }
                ...
                <div className='dashboard-container'>
                    <div className='row'>
                        <div className='box'>
                            ...
                            <h3>{totalBloodrequestd} Units</h3>
                        </div>
                        <div className='box'>
