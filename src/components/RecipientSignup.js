...
function RecipientSignup() {
  ...
  const [recipientsData, setRecipientsData] = React.useState(getLocalStorageItems)
  ...
  function handleSubmit(event) {
    ...
    if (isValid(recipientsData, formData)) {
  ...
  useEffect(() => {
    if(isAuthenticated===true) navigate(`../${user}`);
  }, [isAuthenticated])

  useEffect(() => {
    localStorage.setItem(`${user}`, JSON.stringify(recipientsData))
  }, [recipientsData])
      ...
      <div className='login'>
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          <div className='login-footer'>
            ...
            <Link to={`/${user}`} style={{ 'textDecoration': 'none' }}><span style={{ 'fontSize': '0.9rem', 'color': 'blue' }}>Already registered?</span></Link>
          </div>
        </form>
      </div>
    </>
